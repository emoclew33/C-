...
public:
	...
	ForwardList(unsigned int size) :ForwardList()
	{
		...
		{
			push_front(T());  //T() - значение по умолчанию
		}
	}
...
void main()
{
	...
	for (string i : s_list2) cout << i << tab;
#endif // RANGE_BASE_FOR_LIST
	ForwardList<int> list = { 1,2,2,3,3,4,5 };
	list.unique();
	list.print();
}

// value последовательно принимает значение всех элементов контейнера
//Шаблоны класса
// 
// 
// Шаблоным называется класс, тип полей которого опеределяется при создании обьекта. То есть переменные членны класса могут ыть абсолютно любого типа.
// Но при создании обьекта обязательно нужно указывать тип ео полей. <int>
// *****************************************************************//
//		Class object;		//создание обьекта обычного класса		//
//		Class<type> object; //создание обьекта шаблонного обьекта	//
//																	//
//		template<typename T>										//
// *****************************************************************//
// 
// для того чтобы сделать класс шаблонным, перед ним нужно просто создать шаблон.
// после этого любая переменая может быть шаблонной, а все методы становятся ватоматически - шаблонными
// Синтаксически шаблонизировать моэно любой класс, но не всегда это уместно, поэтому шаблонихируют контейнеры.
// 
//